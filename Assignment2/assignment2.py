...
def newList(x, sizeList):   # this function returns a new list 
    ...
    for key in sizeList.keys():
        a = sizeList.get(key)[0]    #a, b, c, and d values from the parameter list: girth, height, or weight
        b = sizeList.get(key)[1]
        c = sizeList.get(key)[2]
        d = sizeList.get(key)[3]
        
        #trapezoidal values function if statement given from the assignment specs
        if (x <= a):
            newDictionary[key] = 0
        elif ((a < x) and (x < b)):
...
def fuzzy_classifier(input):
    ...
    girthList = newList(input[0], prob.fuzzyGirth)      #returns new list of girth given the input girth and the trapezoidal values for girth
    heightList = newList(input[1], prob.fuzzyHeight)    #returns new list of height given the input height and the trapezoidal values for height
    weightList = newList(input[2], prob.fuzzyWeight)    #returns new list of weight given the input weight and the trapezoidal values for weight

    #AND -> t-norm
    #OR -> s-norm 
    #fuzzy rules for each dog from the assignment specifications
    beagle = tNorm(heightList.get('medium'), sNorm(girthList.get('small'), weightList.get('light')))    #IF height is medium AND (girth is small OR weight is small) THEN beagle
    corgi = tNorm(tNorm(girthList.get('medium'), heightList.get('short')), weightList.get('medium'))    #IF girth is medium AND height is short AND weight is medium THEN corgi.
    husky = tNorm(tNorm(girthList.get('large'), heightList.get('tall')), weightList.get('medium'))      #IF girth is large AND height is tall AND weight is medium THEN husky.
    poodle = tNorm(sNorm(girthList.get('medium'), heightList.get('medium')), weightList.get('heavy'))   #IF (girth is medium OR height is medium) AND weight is large THEN poodle.

    #new list with membership values after fuzzy rule calculations 
